...
export async function main(ns) {
    ...
    do {
        ...
        if (maxAffordableRam < maxOwnedRam && maxAffordableRam < maxGameRam) {
            log(ns, `WARNING: Not purchasing a server with ${formatRam(ns, maxAffordableRam)} RAM for ${formatMoney(ns, maxAffordableCost)} as it is less than the maximum owned server RAM (${formatRam(ns, maxOwnedRam)}).`, verbose);
