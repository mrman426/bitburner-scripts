...
export function autocomplete(data, args) {
    if (args.length === 0) {
        return ["--all-servers", "--purchased-only", "--targets"];
    }
    return data.servers;
}
...
export async function main(ns) {
    ...
    const targetListArg = ns.args.find(arg => arg.startsWith("--targets="));
    let targetServers = null;

    if (targetListArg) {
        // Extract and parse the list of target servers
        const targetList = targetListArg.split("=")[1];
        targetServers = targetList.split(",").map(server => server.trim());
    }
